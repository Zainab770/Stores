...
const Zainab = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("http://localhost:9500/fruitfetch"); // API call for fetching fruits
        if (!response.ok) {
        ...
        const updatedData = data
          .filter((item) => item.category === "Fruits & Vegetables") // Filtering by category
          .map((item) => ({
            ...item,
        ...
        {isMenuOpen && (
          <aside style={{ width: "280px", backgroundColor: "#fefefe", padding: "40px 30px", border: "2px solid green", boxShadow: "2px 0 8px rgba(0, 0, 0, 0.05)", height: "auto" }}>
            ...
            <ul style={{ paddingLeft: "20px", margin: 0, listStyleType: "disc" }}>
              {[
                "Roots Vegetables", "Leafy Greens & Herbs", "Fruiting Vegetables",
                "Cruciferous Vegetables", "Tropical Fruits", "Citrus & Melons",
                "Berries", "Stone & Pome Fruits",
              ].map((item, index) => (
